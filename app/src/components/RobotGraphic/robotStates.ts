...
import type { RobotState, AnimationConfig, RobotStateKey, SizeConfig } from './types'

/**
 * Robot state definitions mapping application states to robot expressions and behaviors
 */
export const ROBOT_STATES: Record<'IDLE' | 'THINKING' | 'TALKING' | 'ERROR', RobotState> = {
  IDLE: {
    key: 'idle',
    expression: 'happy',
 ...
 * @returns The robot state object
 */
export function getRobotState(stateKey: string): RobotState {
  const stateMap: Record<RobotStateKey, RobotState> = {
  ...
  return stateMap[stateKey as RobotStateKey] || ROBOT_STATES.IDLE
}

/**
 * Utility function to validate robot state key
 * @param stateKey - The state key to validate
 * @returns Whether the state key is valid
 */
export function isValidRobotState(stateKey: string): stateKey is RobotStateKey {
